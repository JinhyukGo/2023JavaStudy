...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		oBox1.store(orange1);
		// getter를 통해 반환받은 후 정보를 출력
		orange1 = oBox1.pullOut();
		orange1.showInfo();
		
		/*
		상자1은 오렌지 전용박스이므로 다른 과일(즉 인스턴스)를 저장할 수 없음
		컴파일 에러가 발생하게 되므로 실행 자체가 불가능해짐
		즉 자료형에는 아넌하지만, 구현에는 불편함이 있음
		다른 인스턴스를 저장하기 위해서는 또 다른 Box 클래스를 생성해야 함
		 */
		
		// 아래와 같이 Orange가 아닌 인스턴스를 저장하면 즉시 컴파일 에러가 발생하여 실행할 수 없는 코드가 됨
		// 즉 자료형에는 안전하다고 할 수 있음
		// oBox1.store("당도가 20인 오렌지");
		Orange orange2 = oBox1.pullOut();
		orange2.showInfo();
		
		/*
		Object 기반으로 생성된 FruitBox는 구현에는 편리하나 코드레벨에서 에러가 발생하지 않으므로 오류를 찾아내기가 훨씬 더 어려움
		 */
		...
		/*
		하지만 오렌지 같은 String을 저장하고, 이를 꺼내서 다운캐스팅하는 부분에서는 예외가 발생함
		즉 런타임 에러가 발생하게 되므로 실행 전에는 오류를 예측하기 쉽지 않은 단점이 있음 
		 */
