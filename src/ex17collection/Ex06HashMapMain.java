...
- Key, Value의 쌍으로 인스턴스를 저장함
- key는 중복될 수 없음 만약 중복되면 기존의 데이터를 덮어쓰기함
- key를 통해 검색되므로 다른 컬렉션에 비해 속도가 빠름
- 저장 순서는 유지되지 않음
 */
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		Map 컬렉션 생성.
		Key와 Value 모두 String으로 선언함
		 */
		HashMap<String, String> map = new HashMap<String, String>();
		
		/*
		객체 저장(인스턴스 저장) : 최초 저장이라면 성공시 null을 반환함
		만약 기존에 이미 저장된 동일한 key가 존재하면 저장된 인스턴스가 반환됨
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값 : " + map.put("name", "홍길동"));
		
		int number = 20;
		// int형은 String으로 변환할 수 없으므로 타입 에러 발생
		// map.put("age", number);
		...
		/*
		중복 저장 : 기존에 입력된 name이라는 Key가 있으므로 "홍길동"이 반환됨
		...
		/*
		출력하기
		1. Key를 알고 있을 때 : get(key)으로 출력
		 */
		System.out.println("키값을 알 때 : " + map.get("name"));
		
		/*
		출력하기
		2. Key를 모를 때(혹은 너무 많아서 하나씩 명시하기 힘들 때) : KeySet()을 통해 전체 Key를 얻어옴
		이를 통해 for문으로 순차적인 접근을 하면 됨
		 */
		...
		/*
		3. 이터레이터를 통한 출력 : 이터레이터를 통해 출력할 때도 keySet()을 통해 전체 Key를 얻어온 후 반복출력함
		 */
		...
		/*
		Value만 얻어올 때는 values()를 통해 값을 인출할 수 있음
		반환되는 값은 Collection<T>타입
		 */
		...
		/*
		인스턴스의 존재 유무 확인 : Map 컬렉션은 Key와 Value가 있으므로 메서드도 아래와 같이 2개로 나뉘어 있음 
		 */
		...
		/*
		인스턴스 삭제 : Key를 통해 삭제하고, 삭제에 성공하면 해당 Value를 반환함
		 */
		...
		/*
		전체 삭제 : removeAll()은 없고, clear()만 사용할 수 있음
		 */
		map.clear();
