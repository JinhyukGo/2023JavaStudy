...
class ChildPropertyT {
	...
	// 멤버변수 : 어린이가 보유하고 있는 구슬의 개수
	int beads;
	
	// 생성자는 외부에서 호출해야 하므로 반드시 public으로 지정
	public ChildPropertyT(int beads) {
		this.beads = beads;	
	}
	
	// 반환값없이 단순히 출력
	void showProperty() {
		System.out.println("보유구슬의 개수 : " + this.beads);
	}
	
	void obtainBead(ChildPropertyT cp, int bCnt) {
		// 승리한 어린이 합산 // 호출의 주체가 되는 인스턴스
		this.beads += bCnt;
