...
/*
가변인자(Variable Argument) : 메서드 오버로딩을 대체할 수 있는 문법으로 하나의 메서드로 여러 가지의 호출 형식을 대체할 수 있음
형식
 - 메서드명(자료형 ... 매개변수명) {
 		실행문장;
   }
...
public class E08VariableArgument {
	...
	// 앞에서 학습한 메서드 오버로딩을 통한 메서드 정의
	// 계속해서 반복해야 하므로 개수가 많아질 경우 곤란해짐
...
//	}
	...
	// ...을 통해 매개변수 param이 2개 이상의 인수를 받을 수 있는 것을 명시함
	// 또한 메서드 내에서는 배열로 반환되어 "매개변수명.length"와 같이 사용할 수 있음
	static int getTotal(int ... param) {
		int total = 0;
		// 전달된 매개변수의 크기만큼 반복 누적하여 더함
	...
	public static void main(String[] args) {
		...
		// 가변 인자를 통해 메서드를 정의하였으므로 아래 모든 호출 문장은 정상적으로 실행됨
		...
		// 매개변수의 타입에 double이 포함되어 있으므로 호출할 수 없음
		// 만약 아래와 같이 실수를 모두 처리하려면 가변인자의 타입을 double로 선언하면 됨
